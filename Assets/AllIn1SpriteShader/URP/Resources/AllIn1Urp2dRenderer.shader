Shader "AllIn1SpriteShader/AllIn1Urp2dRenderer"
{
    Properties
    {
		...
		[HideInInspector] _MinXUV("_MinXUV", Range(0, 1)) = 0.0 //155
		...
		[HideInInspector] _RandomSeed("_MaxYUV", Range(0, 10000)) = 0.0 //159

		_MaskTex("Lighting Mask", 2D) = "white" {} //160
		_LitAmount("Lit Amount",  Range(0,1)) = 1 //161
		_NormalMap("Normal Map", 2D) = "bump" {} //162
		_NormalStrength("Normal Strength",  Range(0,10)) = 1 //163
		[Toggle()] _GlowAffectLight("Glow affects light", Float) = 1 //164
    }
    ...
    SubShader
    {
        Tags {"Queue" = "Transparent" "RenderType" = "Transparent" "RenderPipeline" = "UniversalPipeline" "CanUseSpriteAtlas" = "True" "PreviewType" = "Plane"}
        ...
        Pass //Main Light Shapes pass
        {
            ...
            struct v2f
            {
				...
				#endif
				#if FADE_ON
				half2 uvFadeTex1 : TEXCOORD5;
				half2 uvFadeTex2 : TEXCOORD6;
				#endif
            };
			...
			half _Alpha, _RandomSeed, _LitAmount;
            ...
            v2f CombinedShapeLightVertex(Attributes v)
            {
				...
				o.vertex = TransformObjectToHClip(half4(localPos, 1));
				#else
		...
		Pass //Normal Mapping
        {
            ...
            struct v2f
            {
				...
				#endif
				#if FADE_ON
				half2 uvFadeTex1 : TEXCOORD7;
				half2 uvFadeTex2 : TEXCOORD8;
				#endif
            };
			...
			half _Alpha, _RandomSeed, _NormalStrength;
			...
			sampler2D _FadeTex, _FadeBurnTex;
			half4 _FadeBurnColor, _FadeTex_ST, _FadeBurnTex_ST;
			half _FadeAmount, _FadeBurnWidth, _FadeBurnTransition;
			#endif
			...
			#if OUTBASE_ON
			half4 _OutlineColor;
			half _OutlineAlpha, _OutlineWidth;
			int _OutlinePixelWidth;
			#endif
			...
			#if INNEROUTLINE_ON
			half _InnerOutlineThickness, _InnerOutlineAlpha;
			half4 _InnerOutlineColor;
			#endif
            ...
            float4 NormalsRenderingFragment(v2f i) : SV_Target
            {
				...
				#if !ONLYINNEROUTLINE_ON
				innerT = (innerT / 2.0) * col.a * _InnerOutlineAlpha;
				col.rgb += length(innerT) * _InnerOutlineColor.rgb;
				#else
				innerT *= col.a * _InnerOutlineAlpha;
				col.rgb = length(innerT) * _InnerOutlineColor.rgb;
				col.a = step(0.3, col.r+col.g+col.b);
				...
				#if BLUR_ON
				#if !BLURISHD_ON
				normalSample = BlurHD(i.uv, _NormalMap, _BlurIntensity);
				#else
